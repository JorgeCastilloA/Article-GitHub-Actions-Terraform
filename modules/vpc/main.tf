...
resource aws_subnet "project_publicC" {
  vpc_id = aws_vpc.project_vpc.id
  cidr_block = cidrsubnet(aws_vpc.project_vpc.cidr_block, 8, 4)
  availability_zone = var.availability-zone-A

  tags = {
    "Name" = "project_publicC"
  }
}

resource aws_subnet "project_privateC"{
  vpc_id = aws_vpc.project_vpc.id
  cidr_block = cidrsubnet(aws_vpc.project_vpc.cidr_block, 8, 5)
  availability_zone = var.availability-zone-B

  tags = {
    "Name" = "project_privateC"
  }
}
